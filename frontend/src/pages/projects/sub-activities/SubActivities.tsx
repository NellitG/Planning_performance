...
import { Plus, Search, ChevronLeft, ChevronRight, Eye, Pencil, Trash2, Layers2 } from "lucide-react";
import { toast } from "sonner";
import { PageHeader } from "@/components/PageHeader";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { useSubActivities, useDeleteSubActivity } from "@/hooks/useProjectsApi";
        ...
        actions={
          <Button asChild className="bg-primary text-primary-foreground hover:bg-primary/90">
            <Link to="/projects/sub-activities/new">
              <Plus className="h-4 w-4" /> Add New Sub Activity
            </Link>
          </Button>
        }
      />
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm">
        <div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span className="text-sm text-muted-foreground">
            {filtered.length} sub activit{filtered.length !== 1 ? "ies" : "y"}
          </span>
        </div>
