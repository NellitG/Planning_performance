  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Manage Main Activitiess. Each Main Activities can have strategic objectives and strategies."
        actions={
          <Button asChild className="bg-primary text-primary-foreground hover:bg-primary/90">
            <Link to="/reports/mainactivities">
              <Plus className="h-4 w-4" /> Create Main Activities
            </Link>
          </Button>
        }
      />
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm">
        <div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span className="text-sm text-muted-foreground">{filtered.length} Main Activities{filtered.length !== 1 ? "s" : ""}</span>
        </div>
              ...
              {pageItems.length === 0 && (
                <TableRow>
                  <TableCell colSpan={4} className="py-12 text-center text-sm text-muted-foreground">
                    {q ? "No Main Activitiess match your search." : "No Main Activitiess yet. Click 'Create Main Activities' to get started."}
                  </TableCell>
                </TableRow>
              )}
